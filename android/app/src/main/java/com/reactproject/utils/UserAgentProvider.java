 ...
 *
 *      cangkrus/1.0 (Android 4.2.2; Genymotion Vbox86p / Generic Galaxy Nexus - 4.2.2 - API 17 - 720x1280; )[preload=false;locale=en_US;clientidbase=]
 *
...
public class UserAgentProvider implements Provider<String> {
    ...
    private static final String APP_NAME = "bixbox";
            ...
            synchronized (UserAgentProvider.class) {
                if (userAgent == null) {
                    userAgent = String.format("%s/%s (Android %s; %s %s / %s %s; %s)",
                            ...
                            WordUtils.capitalize(Build.DEVICE),
                            WordUtils.capitalize(Build.BRAND),
                            WordUtils.capitalize(Build.MODEL),
                            WordUtils.capitalize(telephonyManager == null ? "not-found" : telephonyManager.getSimOperatorName())
                    );
                    ...
                    if (params.size() > 0)
                        userAgent += "[" + TextUtils.join(";", params) + "]";
