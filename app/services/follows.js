...
export default {
  ...
  search: name => api.get(`/api/users?following=false&search=${name}&searchFields=name_first:like;name_last:like;email:like`),
  searchFollower: (name, leader_id) => api.get(`/api/follows?leader_id=${leader_id}&status=approved&search=${name}&orderBy=users:follower_id|name_first&searchFields=follower.name_first:like;follower.name_last:like;follower.email:like`),
