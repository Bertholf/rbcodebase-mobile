    ...
    */
    if (typeof this.props.user_id === 'undefined') {
      AsyncStorage.getItem('userId')
        .then((myId) => {
          follows
            .searchFollowing(this.state.name, myId)
            .then((res) => {
      ...
      follows
        .showFollowing(this.props.user_id)
        .then((res) => {
          this.changeState(res);
        })
        .catch(err => this.showError(err));
        ...
        <Container>
          ...
          {this.state.wait
            ...
            : <ListView
              ...
              renderRow={rowData => (
                <ListFollow
                  rowData={{ ...rowData, type: 'following', rerender: () => this.rerender() }}
                />
                )}
            />}
        </Container>
            ...
            */}
              {this.state.someone !== '' && this.state.search === 1
                  ? <Text style={styles.textWait}>
                    {strings.listfollow.nofollowing} {this.state.someone}
                  </Text>
                  : <Text style={styles.textWait}>
                    {strings.listfollow.nofollowing}
                  </Text>}
              <TouchableOpacity
