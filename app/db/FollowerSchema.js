...
async function getData() {
  ...
  const follow = await follows.searchFollower('', id);
  return follow;
}
...
const mapFollowerToDatabase = (tx) => {
  ...
  getData().then(async (resp) => {
    console.log(resp);
    const arraysFollowers = await resp.data;
    // const arraysLeader = resp.data.leader;
    const follower = arraysFollowers[0].follower;

    // console.log('follower', follower);

    console.log('LEADER');
    let structureFollowers = '';
    let structureLeader = '';
    ...
    await arraysFollowers.map(i => structureFollowers += `(${i.id}, ${i.leader_id}, ${i.follower_id}, "${i.status}", "${i.created_at}", "${i.updated_at}", "${i.deleted_at}"), `);

    await arraysFollowers.map(i => structureLeader += `(${i.leader.id}, "${i.leader.name_first}", `
      + `"${i.leader.name_last}", `
      + `"${i.leader.name}", `
      + `"${i.leader.name_slug}", `
      + `"${i.leader.email}", `
    ...
    const structFollowers = structureFollowers.substring(0, structureFollowers.length - 2);
    ...
    SQLite.openDatabase(dbName, dbVersion, dbDisplayName, -1).then((ds) => {
      ds.executeSql(`INSERT INTO Followers Values ${structFollowers};`);
...
const createSchemaDb = async (tx) => {
  ...
  // Followers Table
  tx.executeSql('CREATE TABLE IF NOT EXISTS Followers( '
...
const queryFollowers = (tx) => {
  console.log('QUEWRY Followers');
  tx.executeSql('SELECT * FROM Followers ORDER BY id').then(([tx, result]) => {
...
const populateDatabase = (db) => {
  ...
  db.executeSql('SELECT * FROM Version LIMIT 1').then(() => {
    db.transaction(queryFollowers).then(() => {
      closedatabase();
    });
  }).catch((error) => {
    console.log(error);
